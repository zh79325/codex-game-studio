  ...
  const columns: ColumnsType<AiModelUsage> = [
    {
      ...
      render: (_, item) => (
        <Space orientation="vertical" size={1}>
          ...
          <Space wrap>
            {!item.providerEnabled || !item.enabled ? (
              <Tag color="default">已停用</Tag>
            ) : (
              <Tag color="success">已启用</Tag>
            )}
            {item.hasKey ? (
              <Tag color="blue">Key 已配置</Tag>
            ) : (
              <Tag color="warning">缺少 Key</Tag>
            )}
    ...
    },
    {
      ...
      render: (_, item) => (
        <Space wrap>
          {item.agents.length ? (
            item.agents.map((agent) => <Tag key={agent}>{agent}</Tag>)
          ) : (
    ...
    },
    {
      ...
      render: (_, item) =>
        item.budgets.length ? (
          ...
          <Typography.Text type="secondary">未设置限额</Typography.Text>
        ),
    },
    {
      title: "熔断",
      ...
      render: (_, item) =>
        item.breaker ? (
          <Space orientation="vertical" size={2}>
            ...
            {item.breaker.lastReason && (
              <Tooltip title={item.breaker.lastReason}>
                <Typography.Text ellipsis className="breaker-reason">
                  {item.breaker.lastReason}
                </Typography.Text>
              </Tooltip>
            )}
            {item.breaker.retryAt && (
              <Typography.Text type="secondary">
                恢复：{new Date(item.breaker.retryAt * 1000).toLocaleString()}
              </Typography.Text>
            )}
            <Button
              size="small"
              ...
              disabled={!canWrite}
              loading={clearBreaker.isPending}
              onClick={() => clearBreaker.mutate(item.providerModelId)}
            >
              手动放行
            </Button>
          </Space>
    ...
    },
    {
      ...
      render: (_, item) => (
        <Popconfirm
          title="清理该模型全部本地用量？"
          onConfirm={() => reset.mutate({ modelId: item.providerModelId })}
        >
          <Button
            danger
            type="text"
    ...
    <div className="page-stack">
      <section className="page-heading">
        <div>
          ...
          <Typography.Text type="secondary">
            每 10 秒刷新本地 ledger。当前 Provider
            驱动不支持远程对账，因此所有数据均标记为本地来源。
          </Typography.Text>
        </div>
        <Button
          icon={<ReloadOutlined />}
...
function BudgetBar({
  ...
  const percent = budget.unlimited
    ...
    : Math.min(
        100,
        Math.round((budget.used / Math.max(1, budget.limit)) * 100),
      );
  return (
    <div className="budget-row">
      <div className="budget-heading">
        <Space>
          <Tag>{budget.limitKind}</Tag>
          <Typography.Text type="secondary">
            {budget.groupName} · {budget.periodExpr}
          </Typography.Text>
        </Space>
        <Space>
          <Typography.Text>
            {budget.used} / {budget.unlimited ? "∞" : budget.limit}
          </Typography.Text>
          <Button
            size="small"
            type="link"
      ...
      <Progress
        percent={percent}
        status={budget.exhausted ? "exception" : "active"}
        showInfo={!budget.unlimited}
      />
    </div>
  );
}
