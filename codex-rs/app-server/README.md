...
- `game/character/model3d/start` and `game/character/model3d/read` — experimental Codex Game Studio APIs for starting and polling the persisted Tripo game-character pipeline (P1 low-poly generation, rig check, Mixamo rigging, animation retargeting, GLB download and validation). Generation requires confirmed four-view inputs and a project-local `3dModelKeys.tripo3d-zzh` secret.
- `game/aiProvider/*`, `game/aiModel/*`, `game/aiAgent/*`, `game/aiAgentBinding/write`, `game/aiUsage/*`, `game/aiBreaker/clear`, `game/aiConfig/*`, and `game/providerPreset/list` — experimental Codex Game Studio APIs for project-local provider configuration, ordered agent routing, local usage budgets, circuit breakers, secret-free import/export, and the shared recommendation catalog. API keys are accepted only by provider create/update and are never returned.
...
### Example: Configure game AI routing

These endpoints require `capabilities.experimentalApi: true` during initialization. Create a provider with an optional API key, add its model, then write the ordered fallback list for an agent:

```json
{"id":10,"method":"game/aiProvider/create","params":{"provider":{"code":"openai","name":"OpenAI","baseUrl":"https://api.openai.com/v1","driver":"openai","authStyle":"bearer","priority":0,"enabled":true,"remark":"","hasKey":false,"keyMask":null,"models":[]},"apiKey":"sk-...","agentBindings":[]}}
{"id":11,"method":"game/aiModel/create","params":{"model":{"id":"openai:gpt-5","providerCode":"openai","modelId":"gpt-5","displayName":"GPT-5","capabilities":["text_reasoning","text_structured_output"],"driver":"openai","apiPath":"/v1/responses","enabled":true,"sortNo":0,"paramsJson":"{}","remark":"","limits":[{"limitKind":"calls","maxValue":0,"periodExpr":"day","groupName":"default"}]}}}
{"id":12,"method":"game/aiAgentBinding/write","params":{"agentCode":"brief","modelIds":["openai:gpt-5"]}}
```

Provider list responses expose only `hasKey` and `keyMask`. `game/aiConfig/export` also excludes all keys; call `game/aiConfig/import` first with `dryRun: true`, review the returned counts, and repeat with `dryRun: false` to atomically replace the non-secret configuration.
