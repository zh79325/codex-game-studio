...
7. `views` 只调用 `image_i2i`。必须同时引用 `game_context.visualFocus.poseTemplatePath` 的标准姿势模板与 `acceptedRenderPath` 的已确认效果图；系统也会按“姿势模板优先、效果图其次”的顺序把这两张图作为不可移除的强制参考传给图片模型。禁止引用上一张四视图候选，尤其不能用存在朝向、构图或姿势错误的旧候选继续图生图。严格复刻姿势模板的正交 2×2 版式生成单张 2048×2048 图片：左上正面；右上为角色右侧面，脸和鼻尖必须明确朝画面左边；左下背面；右下为角色左侧面，脸和鼻尖必须明确朝画面右边。右上与右下必须像模板一样面对相反方向，一左一右；禁止 30°/45°/三分之二侧视、左右侧面同向或把同一侧视复制到两格。四格必须保持相同角色比例、相同骨骼姿势、相同尺寸、相同垂直位置与地面线，使用标准 T-pose，双臂水平侧平举。背景必须是单一、均匀、不透明的纯色，不得有渐变、暗角、地面、投影或环境光斑；先从角色设定提取主色、辅色、肤色、毛发色与发光色，再选择其中未使用且色相和明度差异最大的高饱和色，并把准确 HEX 写入 prompt。禁止使用角色主色的深浅变体或邻近色作为背景；确保头发、服装、皮肤、尾巴和半透明边缘都能与背景清晰分离，便于色键抠图。禁止武器、手持物、背负装备、环境场景、动作特效和戏剧性姿势，角色本体不可拆除的穿戴结构必须保留。
