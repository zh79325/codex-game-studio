...
desktop-debug:
    ...
    (pnpm --dir {{ justfile_directory() }} --filter @codex-game/desktop dev -- --strictPort & VITE_PID=$!; trap 'kill "$VITE_PID" 2>/dev/null || true' EXIT INT TERM; sleep 1; kill -0 "$VITE_PID" 2>/dev/null || { wait "$VITE_PID"; exit $?; }; RUST_BACKTRACE=1 ELECTRON_ENABLE_LOGGING=1 VITE_DEV_SERVER_URL=http://localhost:5173 CODEX_GAME_WORKSPACE={{ justfile_directory() }} CODEX_GAME_APP_SERVER={{ justfile_directory() }}/codex-rs/target/debug/codex-app-server pnpm --dir {{ justfile_directory() }}/apps/desktop exec electron .)
